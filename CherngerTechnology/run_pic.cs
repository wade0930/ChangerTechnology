...
namespace CherngerTechnology
{
    public partial class run_pic : Form
    {
        ...
        private void _cannyScrollBar1_Scroll(object sender, ScrollEventArgs e)
        {
        ...
        private void _cannyScrollBar2_Scroll(object sender, ScrollEventArgs e)
        {
        ...
        {
            Mat org1 = Cv2.ImRead(@"C:\Users\user\Desktop\t1.jpg", ImreadModes.Color);
            Mat org2 = Cv2.ImRead(@"C:\Users\user\Desktop\t2.jpg", ImreadModes.Color);
            //pictureBox2.Image = org1.ToBitmap();
            //pictureBox3.Image = org2.ToBitmap();
            ...
            pictureBox1.Image = org1.ToBitmap();
        ...
        private void AddXhScrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            ...
            Mat temp = new Mat();
            if (tempX != AddXhScrollBar.Value)
            {
                //temp = new Mat(input, new Rect(tempX, tempY, roi.Width, roi.Height));
                tempP1 = new Mat(temp);
                pictureBox1.Image = tempP1.ToBitmap();
                tempX = AddXhScrollBar.Value;
            }
            
            temp = new Mat(input, new Rect(AddXhScrollBar.Value, AddYhScrollBar.Value, roi.Width, roi.Height));
            tempP1 = new Mat(input2, new Rect(AddXhScrollBar.Value, AddYhScrollBar.Value, roi.Width, roi.Height));
        ...
        private void AddYhScrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            ...
            Roi_formula(pictureBox3, addsrc, ref roi);
            Mat temp = new Mat();
            if (tempY != AddYhScrollBar.Value)
            {
                temp = new Mat(tempP1, new Rect(tempX, tempY, roi.Width, roi.Height));
                //temp = tempP1.Clone();
                pictureBox1.Image = tempP1.ToBitmap();
                tempY = AddYhScrollBar.Value;
            }

            temp = new Mat(input, new Rect(AddXhScrollBar.Value, AddYhScrollBar.Value, roi.Width, roi.Height));
            tempP1 = new Mat(src, new Rect(AddXhScrollBar.Value, AddYhScrollBar.Value, roi.Width, roi.Height));
            Cv2.AddWeighted(roi, _picScrollBar1.Value / (double)10, temp, _picScrollBar2.Value / (double)10, 0, temp);
            pictureBox2.Image = input.ToBitmap();
        }
